...
class EntityHelper
{
  ...
  FROM (SELECT EXCEPTIONAL  FROM  HRIS_DEPARTMENTS WHERE DEPARTMENT_ID IN  ({$depVal}))
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL)");
    ...
    public static function getEmployeeIdFromCode($adapter, $code)
    {
    ...
    public static function getRAWiseEmployeeList($adapter, $eid, $returnType = 0)
    {
        $sql = "SELECT * FROM (select e.employee_id as employee_id, 
                employee_code || '-' ||e.full_name as full_name,
                (case when ra.approved_by = $eid then 'Y' else 'N' end) IS_APPROVER
                from hris_recommender_approver ra
                join hris_employees e on (e.employee_id = ra.employee_id) where Approved_By = $eid
                or Recommend_By = $eid)
                union
                (select e.employee_id as employee_id, employee_code || '-' ||e.full_name full_name,
                (case when Ra.R_A_Flag = 'A' then 'Y' else 'N' end) IS_APPROVER
                from Hris_Alternate_R_A ra
                join hris_employees e on (e.employee_id = ra.employee_id) where r_a_id = $eid)";
        $result = self::rawQueryResult($adapter, $sql);
        $rawData = Helper::extractDbData($result);
        if ($returnType == 1) {
            return $rawData;
        }
        $data = [];
        ...
        if (count($rawData) > 0) {
            for ($i = 0; $i < count($rawData); $i++) {
                $data[$rawData[$i]['EMPLOYEE_ID']] = $rawData[$i]['FULL_NAME'];
                $approver[$rawData[$i]['EMPLOYEE_ID']] = $rawData[$i]['IS_APPROVER'];
            }
        }
        if ($returnType == 2) {
